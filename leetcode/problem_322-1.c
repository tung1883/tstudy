...
/*
 * Key notes:
 * 2 tricks to compute ceil(a/b): (1) (a + b - 1) / b or (2) a / b + (a % b != 0)
 * For the first one, one need to be careful if a + b - 1 can overflow
 */
...
void dfs(int* coins, int n, int idx, int rem, int cnt) {
    if (rem == 0) { if (cnt < best) best = cnt; return; }
    if (idx == n || calls > CALL_LIMIT) return;
    
    int coin = coins[idx];
    if (cnt + rem / coin + (rem % coin != 0) >= best) return; // pruning when the result is worse than the current best

    int maxk = rem / coin;
    for (int k = maxk; k >= 0; --k) {
        calls++;
        dfs(coins, n, idx + 1, rem - k * coin, cnt + k);
    }
}

static int cmp_desc(const void* a, const void* b) {
    int x = *(const int*)a, y = *(const int*)b;
    return (x < y) - (x > y);   // >0 if x<y  -> sorts descending
}
...
int coinChange(int* coins, int coinsSize, int amount) {
    qsort(coins, coinsSize, sizeof(int), cmp_desc);
    best = INT_MAX;
    calls = 0;
    dfs(coins, coinsSize, 0, amount, 0);
    if (calls < CALL_LIMIT) return best == INT_MAX ? -1 : best;
