...
int* manacher_odd(char* s, int len) {
    ...
    for (int i = 1; i <= len; i++) {
        if (i <= r) {
            res[i - 1] = (r - i < res[l + r - i - 1]) ? r - i : res[l + r -  i - 1];
        }

        while (temp[i - res[i - 1] - 1] == temp[i + res[i - 1] + 1]) {
            res[i - 1]++;
        }

        if (i + res[i - 1] > r) {
            l = i - res[i - 1], r = i + res[i - 1];
        }
    }
...
int* manacher(char* s, int len) {
    ...
    temp[2 * len] = '#'; 
    int* odd_arr = manacher_odd(temp, 2 * len + 1);
...
char* longestPalindrome(char* s) {
    ...
    int* manacher_table = manacher(s, len);
    for (int i = 0; i < 2 * len; i++) {
        if (manacher_table[i] > max_len) {
            max_len = manacher_table[i];
            idx = i;
        }
    }

    int startIdx = (idx - max_len) / 2;
